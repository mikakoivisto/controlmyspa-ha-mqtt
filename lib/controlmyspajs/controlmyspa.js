...
class ControlMySpa {
  ...
  async retry(fn, retries = 100, delay = 500) {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        return await fn();
      } catch (err) {
        logDebug(`Attempt ${attempt}/${retries} failed: ${err.message}`);
        if (attempt === retries) {
          logError(`All ${retries} attempts failed: ${err.message}`);
          throw err;
        }
  ...
  async init() {
    const steps = [this.login, this.getProfile, this.getDefaultSpa, this.getSpa];
    logInfo('Starting ControlMySpa initialization');
    for (const step of steps) {
      await this.retry(step.bind(this));
      logInfo(`Completed init step: ${step.name}`);
    }
  ...
  async login () {
    ...
    const req = await axios.post("https://iot.controlmyspa.com/auth/login", body, {
      headers: {
